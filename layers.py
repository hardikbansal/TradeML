...
def general_conv1d(inputconv, num_filters=64, filter_length=2, stride_length=1, stddev=0.02, padding="same", name="conv1d"):
    
    with tf.variable_scope(name) as scope:

        conv = tf.layers.conv1d(inputconv, num_filters, filter_length, stride_length, padding)
        return conv        
